...
uih_cyclinghandler (void *userdata, int n)
{
  ...
  if (direct)
	...
	uih->paletteshift += uih->zengine->fractalc->palette->size - 1;
      uih->paletteshift =
	uih->paletteshift % (uih->zengine->fractalc->palette->size - 1);
      uih->zengine->fractalc->palette->cyclecolors (uih->zengine->
						    fractalc->palette,
						    direct);
...
void
uih_loadfile (struct uih_context *c, xio_constpath d)
{
  xio_file f;
  f = xio_ropen (d);
  if (f == NULL)
...
void
uih_loadexample (struct uih_context *c)
{
  xio_pathdata name;
...
void
uih_savepngfile (struct uih_context *c, xio_constpath d)
{
  CONST char *s;
  if (c->passfunc != NULL)
    {
...
void
uih_saveposfile (struct uih_context *c, xio_constpath d)
{
  xio_file f;
  c->errstring = NULL;
  f = xio_wopen (d);
...
char *
uih_savepostostr (struct uih_context *c)
{
  xio_file f;
  c->errstring = NULL;
  f = xio_strwopen ();
...
void
uih_savecfg (struct uih_context *c)
{
  xio_file f;
  c->errstring = NULL;
...
#endif
void
uih_saveanimfile (struct uih_context *c, xio_constpath d)
{
  xio_file f;
  c->errstring = NULL;
  if (c->save)
...
CONST char *
uih_save (struct uih_context *c, xio_constpath filename)
{
//  FILE *f;
  CONST char *r = NULL;
  uih_cycling_stop (c);
...
uih_do_fractal (uih_context * c)
{
  ...
  if ((c->juliamode == 1
	   ...
	   || c->fcontext->slowmode)) || (c->juliamode == 2
					  && c->fcontext->
					  currentformula->calculate_julia !=
					  NULL && !c->fcontext->slowmode))
    {
      uih_disablejulia (c);
      uih_enablejulia (c);
      c->fcontext->version++;
...
void
uih_text (uih_context * c, CONST char *text)
{
  int i, l;
  c->display = 1;
...
void
uih_emulatetimers (uih_context * c)
{
  if (c->emulator == NULL)
    return;
...
static void
uih_unemulatetimers (uih_context * c)
{
...
void
uih_constantframetime (uih_context * c, int time)
{
  if (c->emulator == NULL)
...
uih_getinstance (CONST struct filteraction *a)
{
